...
def aggregate_proportions(line_list):
    ...
    {'normalize_proportions': 'sum', 'variant__colour': 'first'}).reset_index()
...
def plot_figure(pivot_table, line_list):
    ...
    pivot_table.plot(kind='bar', stacked=True, ax=ax, width=0.8, color=[line_list[line_list['variant']== v]['variant__colour'].unique()[0] for v in pivot_table.columns])
    plt.xlabel('Date')
